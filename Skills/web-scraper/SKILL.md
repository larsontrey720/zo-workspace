...
## CLI
```bash
python3 /home/workspace/Skills/web-scraper/scripts/scraper.py  [options]
```
| Command | Description |
|---------|-------------|
| `scrape ` | Fetch page content as markdown |
| `scrape  --css ` | Extract elements matching a CSS selector |
| `scrape  --schema ` | Extract structured data using a JSON CSS schema |
| `scrape  --llm-extract ""` | Use LLM to extract structured data (Crawl4AI) |
| `crawl  --depth N --max-pages N` | Deep crawl following links (BFS) |
| `batch  [--css ]` | Scrape multiple URLs from a file (one per line) |
## Options
| Flag | Description |
|------|-------------|
...
| `--wait ` | Wait after page load before extracting |
| `--timeout ` | Request timeout (default: 30) |
## Tiered Approach
The scraper automatically picks the lightest tool that can do the job:
1. \*\*Simple fetch\*\* (`scrape `) -- Uses httpx + BeautifulSoup. Fast, no browser overhead.
...
```
## Notes
- Zo's `read\_webpage` and `agent-browser` are still better for quick one-off page reads in conversation. Use this skill when you need structured extraction, deep crawls, or batch operations.
