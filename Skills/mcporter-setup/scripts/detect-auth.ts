...
async function detectAuth(url: string): Promise {
// Normalize URL
...
suggestedSetup:
"Store your API token as a secret (e.g., MY\_API\_TOKEN) and use: --header 'Authorization: Bearer ${MY\_API\_TOKEN}'",
};
}
...
" --header 'x-api-key: ${MY\_API\_KEY}' or\n" +
" --header 'Authorization: Bearer ${MY\_API\_KEY}'",
};
}
...
suggestedSetup:
"Run: npx mcporter config add  " + url + " --scope home",
};
}
}
...
const url = args[0];
if (!url) {
console.log("Usage: bun detect-auth.ts ");
