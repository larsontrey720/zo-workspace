...
--auth-type  Auth type: none, oauth, api-key, bearer, custom
--header-name  Header name for API key (e.g., "x-api-key")
--secret-var  Environment variable name for secret
--scope  Config scope: home (default) or project
--help Show this help
`);
process.exit(0);
}
...
async function addServer(config: ServerConfig): Promise {
